...
说明:	加速度计低通滤波器
入口:	double curr_input 当前输入加速度计,滤波器参数，滤波器缓存
出口:	无
...
////                                         &Butter_Buffer_Feedback[0],&Butter_10HZ_Parameter_Acce);//用作加速度反馈的加速度计量  15
////  Acce_Control_Feedback[1]=LPButterworth(Y_Origion
////                                         ,&Butter_Buffer_Feedback[1],&Butter_10HZ_Parameter_Acce);
////  Acce_Control_Feedback[2]=LPButterworth(Z_Origion
////                                         ,&Butter_Buffer_Feedback[2],&Butter_10HZ_Parameter_Acce);
//
...
/**
 * @note discrete low pass filter, cuts out the high frequency noise
 * that can drive the controller crazy
 * @note derivative = _last_derivative + _d_lpf_alpha * (derivative - _last_derivative);
